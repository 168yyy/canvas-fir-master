window.onload = function () {
  ...
  var huaqipan = function() {
    ...
    for(var i = 0; i < ROW; i++){
      var li = ctx2.createLinearGradient(0,0,560,0);
      li.addColorStop(0.5,'red');
      li.addColorStop(1,'black');
      ctx2.strokeStyle = li;
      ctx2.beginPath();
      ctx2.moveTo(20,i*40 + 20.5);
      ctx2.lineTo(580,i*40 + 20.5);
      ctx2.stroke();

      var li = ctx2.createLinearGradient(0,0,0,560);
      li.addColorStop(0.5,'#333');
      li.addColorStop(1,'#444');
      ctx2.strokeStyle = li;
      ctx2.beginPath();
      ctx2.moveTo(i*40+20.5,20);
      ctx2.lineTo(i*40+20.5,580);
      ctx2.stroke();
    }
    ctx2.fillStyle = 'black';
    ctx2.beginPath();
    ctx2.arc(300.5,300.5,3,0,Math.PI*2);
    ctx2.fill();
    for(var i = 0; i < z.length; i++){
      for(var j = 0; j < z.length; j++){
        ctx2.beginPath();
        ctx2.arc(z[i],z[j],3,0,Math.PI*2);
        ctx2.fill(); 
      }
    }
  }
  ...
  */
  ...
  var luozi1 = function(x,y,color) {
    var zx = 40*x + 2.5;
    var zy = 40*y + 2.5;
    if(color){
      ctx2.drawImage(qiziimg,0,0,42,44,zx,zy,36,36);
    }else{
      ctx2.drawImage(qiziimg,43,0,42,44,zx,zy,36,36);
    }
  }
